...
function getDueBadge(dueDate: Date | null) {
  ...
  return { label: `${daysLeft} days left`, sub: dueDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), bg: 'bg-[#E9E3FF] border-[#D4CAF7]', textColor: 'text-[#8A70D6]', subColor: 'text-[#6B52B8]' }
}
...
function renderDescription(description: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]
    if (line.startsWith('## '))      { elements.push(<h3 key={key++} className="text-[15px] font-bold text-[#424040] mt-4 mb-2 first:mt-0">{line.slice(3)}</h3>) }
    else if (line.startsWith('- '))  { elements.push(<li key={key++} className="text-[13px] text-[#424040] leading-relaxed ml-4 list-disc">{line.slice(2)}</li>) }
    else if (line.trim() !== '')     { elements.push(<p key={key++} className="text-[14px] text-[#424040] leading-[1.7] mb-2">{line}</p>) }
  }
  return elements
}
  ...
  return (
    <div className="p-7 max-w-[900px]">
      ...
      <nav className="flex items-center gap-1.5 text-[12px] text-[#8A8888] mb-6 flex-wrap">
        <Link href="/assignments" className="text-[#8A70D6] hover:underline">Assignments</Link>
        <span>›</span>
        <span className="text-[#424040]">{course.title}</span>
      </nav>

      {/* Header */}
      <div className="flex items-start justify-between gap-4 mb-7 flex-wrap">
        <div>
          <h1 className="text-[22px] font-black text-[#424040] tracking-tight mb-2">
            {assignment.title}
          </h1>
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[12px] text-[#8A8888]">{course.title}</span>
            <span className="text-[#8A8888]">·</span>
            <span className="text-[12px] text-[#8A8888]">{module_.title}</span>
            <span className="text-[#8A8888]">·</span>
      ...
      <div className="grid grid-cols-[1fr_340px] gap-6 items-start">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-white border border-[#EBEBEB] rounded-[14px] p-6 shadow-[0_1px_3px_rgba(0,0,0,.06),0_4px_16px_rgba(0,0,0,.04)]">
            <h2 className="text-[15px] font-bold text-[#424040] mb-4">Assignment Instructions</h2>
            ...
            {requirements.length > 0 && (
              <>
                <h3 className="text-[13px] font-bold text-[#424040] mb-3">Requirements</h3>
                <div className="flex flex-col gap-2 mb-4">
                  {requirements.map((req, i) => (
                    <div key={i} className="flex items-start gap-2.5 text-[13px] text-[#424040]">
                      <span className="w-5 h-5 rounded-full bg-[#E9E3FF] flex items-center justify-center text-[10px] text-[#8A70D6] font-bold flex-shrink-0 mt-0.5">
                        {i + 1}
                      </span>
            ...
            {bonusLines.length > 0 && (
              <div className="bg-[#FBFBFB] rounded-lg p-3.5 border-l-[3px] border-[#8A70D6]">
                <div className="text-[12px] font-bold text-[#8A70D6] mb-1">Bonus Challenge</div>
                {bonusLines.map((b, i) => (
                  <div key={i} className="text-[13px] text-[#8A8888]">{b}</div>
                ))}
              </div>
            )}
          </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-white border border-[#EBEBEB] rounded-[14px] p-6 shadow-[0_1px_3px_rgba(0,0,0,.06)]">
                <div className="flex items-center gap-2.5 mb-4">
                  ...
                  <span className="text-[12px] text-[#8A8888] ml-auto">
                    {new Date(submission.submittedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                  </span>
                </div>
                {submission.githubUrl && (
                  <div className="flex items-center gap-2 text-[13px] text-[#424040] mb-2">
                    <svg className="w-4 h-4 text-[#8A8888]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"/>
                    </svg>
                    <a href={submission.githubUrl} target="_blank" rel="noopener noreferrer" className="text-[#8A70D6] hover:underline truncate">
                      {submission.githubUrl}
                    </a>
                  </div>
                )}
                {submission.liveUrl && (
                  <div className="flex items-center gap-2 text-[13px] text-[#424040]">
                    <svg className="w-4 h-4 text-[#8A8888]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/>
                      <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>
                    </svg>
                    <a href={submission.liveUrl} target="_blank" rel="noopener noreferrer" className="text-[#8A70D6] hover:underline truncate">
                      {submission.liveUrl}
                    </a>
                  </div>
                )}
              ...
              {(submission.aiFeedback || aiFeedbackItems.length > 0) && (
                <div className="bg-white border border-[#EBEBEB] rounded-[14px] overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,.06)]">
                  <div className="px-5 py-4 flex items-center gap-3" style={{ background: 'linear-gradient(135deg,#8A70D6 0%,#6B52B8 100%)' }}>
                    ...
                    <div className="flex flex-col gap-3">
                      {aiFeedbackItems.length > 0 ? (
                        aiFeedbackItems.map((item, i) => {
                          ...
                          const bg   = isPositive ? 'bg-[#DCFCE7]' : isWarning ? 'bg-[#FEF3C7]' : 'bg-[#E9E3FF]'
                          const tc   = isPositive ? 'text-[#16A34A]' : isWarning ? 'text-[#D97706]' : 'text-[#8A70D6]'
                          ...
                          return (
                            <div key={i} className="flex items-start gap-2.5">
                              ...
                              <span className="text-[13px] text-[#424040] leading-relaxed">{item}</span>
                            </div>
                          )
                        })
                      ) : (
                        <p className="text-[13px] text-[#424040] leading-relaxed whitespace-pre-wrap">{submission.aiFeedback}</p>
                      )}
                    </div>
                    <div className="mt-4 pt-4 border-t border-[#EBEBEB] text-[12px] text-[#8A8888]">
              ...
              {(submission.status === 'PENDING' || submission.status === 'AI_REVIEWED') && !submission.feedback && (
                <p className="text-[13px] text-[#8A8888] text-center py-2">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white border border-[#EBEBEB] rounded-[14px] p-5 shadow-[0_1px_3px_rgba(0,0,0,.06)]">
            <div className="text-[13px] font-bold text-[#424040] mb-4">Assignment Details</div>
            <div className="flex flex-col gap-3">
              {/* Status */}
              <div className="flex items-center justify-between text-[13px]">
                ...
                {isSubmitted ? (
                  <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full ${
                    ...
                    submission.status === 'AI_REVIEWED' ? 'bg-[#E9E3FF] text-[#8A70D6]' :
              ...
              {due && (
                <div className="flex items-center justify-between text-[13px]">
                  <span className="text-[#8A8888]">Due date</span>
                  <span className={`font-bold ${!isSubmitted && due < new Date() ? 'text-[#EF4444]' : 'text-[#424040]'}`}>
              ...
              {submission?.grade !== null && submission?.grade !== undefined && (
                <div className="flex items-center justify-between text-[13px]">
                  <span className="text-[#8A8888]">Score</span>
                  <span className="font-bold text-[#16A34A]">{submission.grade} / 100</span>
                </div>
              )}

              <div className="flex items-center justify-between text-[13px]">
                <span className="text-[#8A8888]">Max score</span>
                <span className="font-bold text-[#424040]">100 points</span>
              </div>
            </div>
          </div>
          ...
          {/* Related lesson */}
          <div className="bg-white border border-[#EBEBEB] rounded-[14px] p-5 shadow-[0_1px_3px_rgba(0,0,0,.06)]">
            <div className="text-[13px] font-bold text-[#424040] mb-3">Related Lesson</div>
            <Link
              href={`/courses/${course.id}/learn/${assignment.lesson.id}`}
              className="flex items-center gap-3 p-2 rounded-lg hover:bg-[#FBFBFB] transition-colors no-underline"
            >
              <div className="w-7 h-7 rounded-md bg-[#E9E3FF] flex items-center justify-center flex-shrink-0">
                <svg className="w-3.5 h-3.5 text-[#8A70D6]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <polygon points="5 3 19 12 5 21 5 3"/>
                </svg>
              </div>
              <div>
                <div className="text-[12px] font-bold text-[#424040]">{assignment.lesson.title}</div>
                <div className="text-[11px] text-[#8A8888]">{module_.title}</div>
              </div>
            </Link>
          </div>
        </div>
