...
export default async function AdminCoursesPage() {
  ...
  // ── Per-course: avg quiz score + avg progress ─────────────
  ...
  // Avg progress: completedLessons / (enrolled * totalLessons)
  const progressData = await Promise.all(courses.map(async c => {
    ...
    const avgProgress = c._count.enrollments > 0 && totalLessons > 0
      ? Math.round(completedLessons / (c._count.enrollments * totalLessons) * 100)
      : 0
    // Count students who completed all lessons in this course
    // Proxy: students with completed lessons >= totalLessons for this course
  ...
  const avgQuizAll = allQuizResults.length > 0
    ? Math.round(allQuizResults.reduce((s, q) => s + q.score, 0) / allQuizResults.length)
    : 0
