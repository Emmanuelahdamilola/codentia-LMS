...
export default function AIQuizGenerator({ lessons }: { lessons: Lesson[] }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs font-semibold text-[#1A1523] mb-1">Lesson</label>
        ...
        {lessons.length === 0 && (
          <p className="text-xs text-[#9591A8] mt-1">All lessons already have quizzes.</p>
        )}
      </div>

      <div>
        <label className="block text-xs font-semibold text-[#1A1523] mb-1">Prompt</label>
      ...
      <div>
        <label className="block text-xs font-semibold text-[#1A1523] mb-1">Number of questions</label>
      ...
      {preview && preview.length > 0 && (
        <div className="border-t border-[#E9E7EF] pt-4 space-y-3">
          <p className="text-xs font-bold text-[#1A1523]">Preview ({preview.length} questions)</p>
          {preview.map((q, i) => (
            <div key={i} className="bg-[#FAF8FF] rounded-lg p-3">
              <p className="text-xs font-semibold text-[#1A1523] mb-2">Q{i + 1}: {q.question}</p>
              <div className="space-y-1">
                {q.options.map((opt, j) => (
                  <p key={j} className={`text-[11px] px-2 py-1 rounded ${
                    j === q.correctIndex ? 'bg-green-100 text-green-700 font-semibold' : 'text-[#9591A8]'
                  }`}>
