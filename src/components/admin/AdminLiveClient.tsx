...
function statusBadge(status: string) {
  ...
  if (status === 'SCHEDULED') return { label: 'Upcoming',  bg: '#F4F4F6', text: '#8A8888' }
  if (status === 'COMPLETED') return { label: 'Completed', bg: '#F4F4F6', text: '#8A8888' }
  if (status === 'CANCELLED') return { label: 'Cancelled', bg: '#FEE2E2', text: '#B91C1C' }
  return { label: status, bg: '#F4F4F6', text: '#8A8888' }
}
...
function AttendanceChart({ data }: { data: number[] }) {
  if (!data.length) return <div className="text-[11px]" style={{ color: '#8A8888' }}>No data yet.</div>
  const max = Math.max(...data, 1)
  return (
    <div className="flex items-end gap-1" style={{ height: 48 }}>
      {data.map((v, i) => (
        <div key={i} className="flex-1 rounded-t-sm cursor-pointer transition-colors"
          style={{ height: `${Math.max(4, Math.round((v / max) * 44))}px`, background: '#E9E3FF', minWidth: 4 }}
          onMouseEnter={e => (e.currentTarget.style.background = '#8A70D6')}
  ...
  return (
    <div className="grid grid-cols-7 gap-[3px]">
      {cells.map((day, i) => {
        ...
        let bg = 'transparent', color = '#8A8888', fw = '400', border = '1px solid transparent'
        if (status === 'LIVE')      { bg = '#EF4444'; color = '#fff'; fw = '700' }
        else if (isToday)           { bg = '#8A70D6'; color = '#fff'; fw = '700' }
        else if (status === 'SCHEDULED') { bg = '#E9E3FF'; color = '#6B52B8'; border = '1px solid #D4CAF7'; fw = '700' }
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E8E8EC] shadow-[0_1px_4px_rgba(0,0,0,.07)] overflow-hidden mb-4">
      <div className="flex items-center justify-between px-4 py-3.5 border-b border-[#E8E8EC]">
        <div className="text-[14px] font-black" style={{ color: '#8A70D6' }}>Schedule a Class</div>
      ...
      <div className="p-4 flex flex-col gap-3">
        ...
        <div>
          <label className="text-[11px] font-bold uppercase tracking-[.5px] block mb-1.5" style={{ color: '#8A8888' }}>Session Title</label>
        ...
        <div>
          <label className="text-[11px] font-bold uppercase tracking-[.5px] block mb-1.5" style={{ color: '#8A8888' }}>Course</label>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="text-[11px] font-bold uppercase tracking-[.5px] block mb-1.5" style={{ color: '#8A8888' }}>Date</label>
            <input type="date" value={date} onChange={e => setDate(e.target.value)}
              className="w-full rounded-lg px-3 py-2 text-[13px] outline-none"
              style={{ border: '1px solid #E8E8EC', background: '#F4F4F6', color: '#424040' }} />
          </div>
          <div>
            <label className="text-[11px] font-bold uppercase tracking-[.5px] block mb-1.5" style={{ color: '#8A8888' }}>Time</label>
        ...
        <div>
          <label className="text-[11px] font-bold uppercase tracking-[.5px] block mb-1.5" style={{ color: '#8A8888' }}>Duration</label>
        ...
        <div>
          <label className="text-[11px] font-bold uppercase tracking-[.5px] block mb-1.5" style={{ color: '#8A8888' }}>Meeting Link (Zoom / Meet)</label>
          <input type="url" value={meetingLink} onChange={e => setMeetingLink(e.target.value)}
            placeholder="https://zoom.us/j/..."
            className="w-full rounded-lg px-3 py-2 text-[13px] outline-none"
            style={{ border: '1px solid #E8E8EC', background: '#F4F4F6', color: '#424040' }} />
        </div>
        <div className="rounded-lg px-3 py-2.5 text-[12px]" style={{ background: '#E9E3FF', color: '#6B52B8' }}>
  ...
  return (
    <div className="px-7 py-6 pb-12">
      ...
      <div className="flex items-start justify-between mb-5 flex-wrap gap-3">
        <div>
          <h1 className="text-[20px] font-black tracking-tight" style={{ color: '#424040' }}>Live Class Scheduler</h1>
          <div className="text-[13px] mt-0.5" style={{ color: '#8A8888' }}>Schedule, manage, and track live sessions.</div>
        </div>
      ...
      <div className="grid gap-5" style={{ gridTemplateColumns: '1fr 320px' }}>
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="px-4 py-3">
              <div className="grid grid-cols-7 mb-1.5">
                {['M','T','W','T','F','S','S'].map((d, i) => (
                  <div key={i} className="text-[10px] font-bold text-center py-1" style={{ color: '#8A8888' }}>{d}</div>
                ))}
              </div>
              <CalendarGrid classes={classes} />
              <div className="flex gap-4 mt-3 text-[11px]" style={{ color: '#8A8888' }}>
                {[{ color: '#8A70D6', label: 'Scheduled' }, { color: '#EF4444', label: 'Live now' }, { color: '#22C55E', label: 'Completed' }].map(l => (
          ...
          <div className="bg-white rounded-xl border border-[#E8E8EC] shadow-[0_1px_4px_rgba(0,0,0,.07)] overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-[#E8E8EC]">
              <div className="text-[15px] font-black" style={{ color: '#424040' }}>All Sessions</div>
              <div className="flex gap-1 p-1 rounded-lg border border-[#E8E8EC]" style={{ background: '#F4F4F6' }}>
                {(['all','live','upcoming','past'] as const).map(t => (
                  <button key={t} onClick={() => setTabFilter(t)}
                    ...
                    style={{ background: tabFilter === t ? '#fff' : 'transparent', color: tabFilter === t ? '#8A70D6' : '#8A8888',
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid #E8E8EC' }}>
                  {['Title','Date & Time','Course','RSVP','Attended','Recording','Status',''].map(h => (
                    <th key={h} className="text-left text-[11px] font-bold uppercase tracking-[.6px] px-4 py-3" style={{ color: '#8A8888' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} className="text-center py-10 text-[13px]" style={{ color: '#8A8888' }}>No sessions.</td></tr>
                ) : filtered.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="group transition-colors" style={{ borderBottom: '1px solid #E8E8EC' }}
                      ...
                      <td className="px-4 py-3 text-[13px] font-bold" style={{ color: '#424040' }}>{c.title}</td>
                      <td className="px-4 py-3 text-[12px]" style={{ color: '#8A8888' }}>{fmtDate(c.scheduledAt)}</td>
                      <td className="px-4 py-3">
                        <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${coursePillCls(c.courseTitle)}`}>
                          {c.courseTitle.split(' ')[0]}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-[12px] font-bold" style={{ color: '#424040' }}>{c.attendance}</td>
                      ...
                      <td className="px-4 py-3">
                        {c.recordingUrl
                          ...
                          : c.status === 'LIVE' ? <span className="text-[11px]" style={{ color: '#8A8888' }}>Processing...</span>
                          : <span className="text-[11px]" style={{ color: '#8A8888' }}>—</span>}
                      </td>
                      <td className="px-4 py-3">
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-white rounded-xl border border-[#E8E8EC] shadow-[0_1px_4px_rgba(0,0,0,.07)] overflow-hidden">
            <div className="px-4 py-3.5 border-b border-[#E8E8EC]">
              <div className="text-[15px] font-black" style={{ color: '#424040' }}>This Month</div>
            </div>
            <div className="p-4 flex flex-col gap-3">
              {[
                { label: 'Classes held',     value: monthlyStats.classesHeld,                           color: '#424040' },
                { label: 'Total attendees',  value: monthlyStats.totalAttendees,                        color: '#424040' },
                { label: 'Avg attendance',   value: monthlyStats.avgAttendance,                         color: '#22C55E' },
                { label: 'Attendance rate',  value: `${monthlyStats.attendanceRate}%`,                  color: '#8A70D6' },
                { label: 'Recordings ready', value: `${monthlyStats.recordingsReady} / ${monthlyStats.classesHeld}`, color: '#424040' },
              ].map(s => (
                <div key={s.label} className="flex items-center justify-between">
                  <span className="text-[12px]" style={{ color: '#8A8888' }}>{s.label}</span>
              ...
              {monthlyStats.attendanceChart.length > 0 && (
                <div className="pt-3 border-t border-[#E8E8EC]">
                  <div className="text-[12px] font-bold mb-2" style={{ color: '#424040' }}>Attendance per session</div>
          ...
          <div className="bg-white rounded-xl p-7 w-[380px] shadow-2xl" style={{ animation: 'pop .18s ease' }}>
            ...
            <div className="flex gap-2.5">
              ...
              <button onClick={() => setConfirm(null)}
                ...
                style={{ color: '#424040' }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
