...
function typeColor(type: string) {
  if (type === 'QUIZ_RESULT')         return '#8A70D6'
...
const HISTORY = [
  { color: '#8A70D6', text: 'Announcement — "New React course module added" sent to all 248 students',   time: '2h' },
  ...
  { color: '#8A70D6', text: 'Announcement — "Welcome to Codentia! Here\'s how to get started" sent to 8 new students', time: '3 days ago' },
]

// Email stats
const EMAIL_STATS = [
  { label: 'Emails sent',       value: '847', pct: 100, color: '#8A70D6' },
  { label: 'Open rate',         value: '68%', pct: 68,  color: '#22C55E' },
  { label: 'Click-through rate',value: '34%', pct: 34,  color: '#8A70D6' },
...
export default function AdminNotificationsClient({ notifications, unreadCount }: Props) {
  ...
  return (
    <div className="px-7 py-6 pb-12">
      ...
      {/* ── Page header ── */}
      <div className="mb-5">
        <h1 className="text-[20px] font-black tracking-tight" style={{ color: '#424040' }}>Notifications & Email</h1>
      ...
      <div className="grid gap-5" style={{ gridTemplateColumns: '1fr 340px' }}>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white rounded-xl border border-[#E8E8EC] shadow-[0_1px_4px_rgba(0,0,0,.07)] overflow-hidden">
            <div className="px-4 py-3.5 border-b border-[#E8E8EC]">
              <div className="text-[14px] font-black flex items-center gap-2" style={{ color: '#424040' }}>
            ...
            <div className="p-4 flex flex-col gap-4">
              ...
              {/* Target audience */}
              <div>
                <label className="text-[11px] font-bold uppercase tracking-[.5px] block mb-2" style={{ color: '#8A8888' }}>
                  Target Audience
                </label>
                <div className="flex gap-2 flex-wrap">
                  {TARGETS.map(t => (
                    <button key={t.key} onClick={() => setTarget(t.key)}
                      ...
                      style={{
                        background:  target === t.key ? '#8A70D6' : '#fff',
                        color:       target === t.key ? '#fff'    : '#8A8888',
                        borderColor: target === t.key ? '#8A70D6' : '#E8E8EC',
                      }}>
                      {t.label}
                    </button>
                  ))}
              ...
              <div>
                ...
                <textarea value={message} onChange={e => setMessage(e.target.value)}
                  ...
                  style={{ border: '1px solid #E8E8EC', background: '#F4F4F6', color: '#424040', minHeight: 120 }} />
              </div>
              ...
              <div className="flex items-center justify-between px-3 py-2.5 rounded-lg"
                ...
                <div>
                  <div className="text-[13px] font-bold" style={{ color: '#424040' }}>Schedule for later</div>
                  <div className="text-[11px]" style={{ color: '#8A8888' }}>Set a specific send date & time</div>
                </div>
              ...
              <div className="flex gap-2.5">
                <button onClick={handleSend} disabled={sending}
                  className="flex-1 flex items-center justify-center gap-1.5 py-3 rounded-lg font-bold text-[13px] text-white transition-colors hover:bg-[#6B52B8] disabled:opacity-60"
          ...
          <div className="bg-white rounded-xl border border-[#E8E8EC] shadow-[0_1px_4px_rgba(0,0,0,.07)] overflow-hidden">
                ...
                {(['all','unread'] as const).map(t => (
                  <button key={t} onClick={() => setTabFilter(t)}
                    ...
                    style={{
                      ...
                      color:      tabFilter === t ? '#8A70D6' : '#8A8888',
            ...
            <div className="divide-y divide-[#E8E8EC]" style={{ maxHeight: 400, overflowY: 'auto' }}>
              {filtered.length === 0 ? (
                <div className="text-center py-10 text-[13px]" style={{ color: '#8A8888' }}>No notifications.</div>
              ) : filtered.map(n => (
                <div key={n.id} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <div className="text-[13px] font-bold truncate" style={{ color: '#424040' }}>{n.title}</div>
                      {!n.read && <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: '#EF4444' }} />}
                    </div>
                    <div className="text-[12px] mt-0.5 leading-relaxed" style={{ color: '#8A8888' }}>{n.message}</div>
                    <div className="text-[11px] mt-1" style={{ color: '#BCBBBB' }}>
                      {n.userName} · {timeAgo(n.createdAt)}
                    </div>
                  </div>
                  {n.link && (
                    <a href={n.link} className="text-[11px] font-bold flex-shrink-0 hover:underline" style={{ color: '#8A70D6' }}>
                      View →
                    </a>
                  )}
                </div>
            ...
            <div className="px-4 divide-y divide-[#E8E8EC]">
              {[
                ...
                <div key={row.key} className="flex items-center justify-between py-3"
                  ...
                  <div>
                    <div className="text-[13px] font-bold" style={{ color: '#424040' }}>{row.label}</div>
                    <div className="text-[11px]" style={{ color: '#8A8888' }}>{row.sub}</div>
                  </div>
                  <Toggle
                    id={row.key}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white rounded-xl border border-[#E8E8EC] shadow-[0_1px_4px_rgba(0,0,0,.07)] overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-[#E8E8EC]">
              <div className="text-[15px] font-black" style={{ color: '#424040' }}>Notification History</div>
              <span className="text-[11px]" style={{ color: '#8A8888' }}>Last 7 days</span>
            </div>
            <div className="px-3.5 py-1" style={{ maxHeight: 320, overflowY: 'auto' }}>
              {HISTORY.map((h, i) => (
                <div key={i} className="flex items-start gap-2.5 py-3 border-b border-[#E8E8EC] last:border-0">
                  ...
                  <div className="flex-1 text-[12px] leading-relaxed" style={{ color: '#424040' }}
                    dangerouslySetInnerHTML={{ __html: h.text.replace(/^(\w[^—]*)—/, '<strong>$1</strong>—') }} />
                  <div className="text-[11px] flex-shrink-0 whitespace-nowrap" style={{ color: '#8A8888' }}>{h.time}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-[#E8E8EC] shadow-[0_1px_4px_rgba(0,0,0,.07)] overflow-hidden">
            <div className="px-4 py-3.5 border-b border-[#E8E8EC]">
              <div className="text-[15px] font-black" style={{ color: '#424040' }}>Email Stats (7 days)</div>
            </div>
            <div className="p-4 flex flex-col gap-3">
              {EMAIL_STATS.map(s => (
                <div key={s.label}>
                  <div className="flex justify-between text-[12px] mb-1">
                    <span style={{ color: '#8A8888' }}>{s.label}</span>
                    <span className="font-bold" style={{ color: s.label === 'Unsubscribes' ? '#EF4444' : s.label === 'Open rate' ? '#22C55E' : '#424040' }}>
                      {s.value}
                    </span>
                  </div>
