...
// Helpers
// ─────────────────────────────────────────────────────────────
const avatarColors = ['#7C5CDB','#06B6D4','#10B981','#F59E0B','#DC2626','#2563EB']
...
const statusBadge: Record<string, { label: string; bg: string; text: string }> = {
  ...
  completed: { label: 'Completed', bg: '#EDE8FF', text: '#6146C4' },
}
...
const coursePillColor = (title: string) => {
  ...
  if (t.includes('node'))    return { bg: '#F4F4F6', text: '#9591A8' }
  return { bg: '#F4F4F6', text: '#9591A8' }
}
...
function StatCard({ color, icon, delta, deltaDir, value, label }: {
  ...
  const deltaCls = deltaDir === 'up'
    ...
    : 'bg-[#F4F4F6] text-[#9591A8]'
  return (
    <div className="bg-white rounded-2xl border border-[#E9E7EF]" style={{ boxShadow: "0 2px 8px rgba(15,13,26,0.06)" }} className=" relative overflow-hidden" style={{ paddingTop: 3 }}>
      ...
      <div className="p-[18px] pt-[15px]">
        ...
        <div className="text-[28px] font-black leading-none tracking-[-1px] mb-1" style={{ color: '#1A1523' }}>{value}</div>
  ...
  return (
    <div className="px-7 py-6 pb-12">
      ...
      <div className="flex items-start justify-between mb-5 flex-wrap gap-3">
        <div>
          <h1 className="text-[18px] font-semibold tracking-[-0.01em] tracking-tight" style={{ color: '#1A1523' }}>Students</h1>
      ...
      <div className="flex gap-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            ...
            <select value={courseFilter} onChange={e => setCourseFilter(e.target.value)}
              ...
              style={{ height: 34, borderColor: '#E8E8EC', background: '#fff', color: '#1A1523' }}>
            ...
            <button onClick={() => showToast('CSV export started')}
              className="ml-auto flex items-center gap-1.5 px-3 font-bold text-[12px] text-white rounded-lg transition-colors hover:bg-[#6146C4]"
              style={{ height: 34, background: '#7C5CDB' }}>
          ...
          {/* Table */}
          <div className="bg-white rounded-2xl border border-[#E9E7EF]" style={{ boxShadow: "0 2px 8px rgba(15,13,26,0.06)" }} className=" overflow-hidden">
            <table className="w-full">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} className="text-center py-10 text-[13px]" style={{ color: '#9591A8' }}>No students found.</td></tr>
                ) : filtered.map(s => {
                  const { label, bg, text } = statusBadge[s.status]
                  const progressFill = s.status === 'at-risk' ? '#F59E0B' : s.status === 'completed' ? '#22C55E' : '#7C5CDB'
                  ...
                  return (
                    <tr key={s.id} className="group transition-colors cursor-default"
                      ...
                      onMouseEnter={e => (e.currentTarget.style.background = 'var(--color-primary-faint)')}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="text-[13px] font-bold" style={{ color: '#1A1523' }}>{s.name}</div>
                            <div className="text-[11px]" style={{ color: '#9591A8' }}>{s.email}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          ...
                          {s.courseCount > 2 && (
                            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-[#F4F4F6] text-[#9591A8]">
                              +{s.courseCount - 2}
                            </span>
                          )}
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2" style={{ minWidth: 100 }}>
                          ...
                          <span className="text-[11px] font-bold whitespace-nowrap" style={{ color: '#1A1523' }}>
                            {progressPct}%
                          </span>
                        </div>
                      </td>
                      ...
                      {/* Quizzes */}
                      <td className="px-4 py-3">
                        <span className="text-[12px] font-bold" style={{ color: '#1A1523' }}>{s.quizCount}</span>
                        {s.avgQuiz !== null && (
                          <span className="text-[11px] ml-1.5" style={{ color: s.avgQuiz < 60 ? '#EF4444' : '#9591A8' }}>
                            avg {s.avgQuiz}%
                          </span>
                        )}
                      </td>
                      ...
                      {/* Assignments */}
                      <td className="px-4 py-3 text-[12px] font-bold" style={{ color: '#1A1523' }}>
                        {s.assignmentsLabel}
                      </td>

                      {/* Last active */}
                      <td className="px-4 py-3 text-[12px]" style={{ color: '#9591A8' }}>
        ...
        {selected && (
          <div className="flex-shrink-0" style={{ width: 360, animation: 'slideIn .22s ease' }}>
            ...
            <div className="bg-white rounded-2xl border border-[#E9E7EF]" style={{ boxShadow: "0 2px 8px rgba(15,13,26,0.06)" }} className=" overflow-hidden sticky top-0">
              ...
              <div className="relative p-5 pb-4"
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-[52px] h-[52px] rounded-full flex items-center justify-center text-[16px] font-semibold tracking-[-0.01em] text-white flex-shrink-0"
                    style={{ background: 'rgba(255,255,255,.25)' }}>
                    {getInitials(selected.name)}
                  </div>
                  <div>
                    <div className="text-[15px] font-semibold text-white">{selected.name}</div>
              ...
              <div className="p-4 flex flex-col gap-4 max-h-[calc(100vh-260px)] overflow-y-auto">
                ...
                {/* Enrolled courses */}
                <div>
                  <div className="text-[12px] font-bold mb-2" style={{ color: '#1A1523' }}>Enrolled Courses</div>
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {selected.courses.length === 0 && <span className="text-[12px]" style={{ color: '#9591A8' }}>No enrollments</span>}
                  </div>
                </div>

                {/* Recent quiz scores */}
                <div>
                  <div className="text-[12px] font-bold mb-2" style={{ color: '#1A1523' }}>Recent Quiz Scores</div>
                  {selected.recentQuizzes.length === 0 ? (
                    <p className="text-[12px]" style={{ color: '#9591A8' }}>No quizzes taken yet.</p>
                  ) : (
                    <div className="flex flex-col gap-1.5">
                      {selected.recentQuizzes.map((q, i) => (
                        <div key={i} className="flex justify-between text-[11px]">
                          <span style={{ color: '#9591A8' }}>{q.title}</span>
                          <span className="font-bold" style={{
                            color: q.score >= 80 ? '#22C55E' : q.score >= 60 ? '#1A1523' : '#F59E0B',
                          }}>
                            {q.score}%
                          </span>
                        </div>
                ...
                <div>
                  <div className="text-[12px] font-bold mb-1.5" style={{ color: '#1A1523' }}>Live Class Attendance</div>
                  <div className="text-[12px]" style={{ color: '#9591A8' }}>
                    Attended{' '}
                    <strong style={{ color: '#1A1523' }}>{selected.liveAttended} of {selected.totalLive}</strong>
                ...
                {/* Instructor notes */}
                <div>
                  <div className="text-[12px] font-bold mb-1.5" style={{ color: '#1A1523' }}>Instructor Notes</div>
                  <textarea
                    value={notes}
                    ...
                    style={{ border: '1px solid #E8E8EC', background: '#F4F4F6', color: '#1A1523', minHeight: 60 }}
                  />
                </div>
