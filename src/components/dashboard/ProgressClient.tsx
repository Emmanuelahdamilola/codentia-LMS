...
function getCourseThumb(title: string) {
  ...
  if (t.includes('javascript') || t.includes('js')) return { bg: 'linear-gradient(135deg,#F0DB4F,#E8C41A)', icon: '⚡', bar: 'linear-gradient(to right,#7C5CDB,#6146C4)' }
  if (t.includes('react')) return { bg: 'linear-gradient(135deg,#61DAFB,#21A1C4)', icon: '⚛️', bar: 'linear-gradient(to right,#61DAFB,#21A1C4)' }
  return { bg: 'linear-gradient(135deg,#7C5CDB,#6146C4)', icon: '📚', bar: 'linear-gradient(to right,#7C5CDB,#6146C4)' }
}

function heatmapColor(v: number) {
  return ['#E9E7EF','#C4B5FD','#A78BFA','#7C3AED','#6146C4'][v]
}
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 360, damping: 30 }} className="p-4 md:p-7">
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-[26px] font-semibold text-[#1A1523] mb-1 animate-fade-up" style={{ letterSpacing: "-0.03em" }}>My Progress</h1>
          <p className="text-[13.5px] text-[#9591A8] mb-6 animate-fade-up" style={{ animationDelay: "40ms" }}>Track your learning journey, quiz performance, and study habits.</p>

          {/* Stat cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-7">
            {[
              { icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>, color: 'bg-[#EDE8FF] text-[#7C5CDB]', value: stats.lessonsCompleted, label: 'Lessons Completed', change: '↑ this week' },
              ...
              <div key={label} className="bg-white border border-[#E9E7EF] rounded-xl p-4 shadow-[0_2px_8px_rgba(15,13,26,0.06)]">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center mb-2 ${color}`}>{icon}</div>
                <div className="text-[22px] font-semibold text-[#1A1523] tracking-tight leading-none" style={{ letterSpacing: "-0.025em" }}>{value}</div>
                <div className="text-[12px] text-[#9591A8] mt-0.5">{label}</div>
                <div className="text-[11px] font-bold text-[#16A34A] mt-0.5">{change}</div>
              </div>
            ))}
          </div>

          {/* Course Progress */}
          <div className="text-[17px] font-semibold text-[#1A1523] mb-4 animate-fade-up" style={{ letterSpacing: "-0.02em" }}>Course Progress</div>
          <div className="flex flex-col gap-3 mb-6">
            {progressList.length === 0 ? (
              <div className="bg-white border border-[#E9E7EF] rounded-2xl p-10 text-center text-[13px] text-[#9591A8]">
              ...
              return (
                <div key={p.courseId} className="bg-white border border-[#E9E7EF] rounded-2xl p-5 shadow-[0_2px_8px_rgba(15,13,26,0.06)] animate-fade-up">
                  <div className="flex items-center gap-3.5 mb-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[14px] font-bold text-[#1A1523] truncate">{p.courseTitle}</div>
                      <div className="text-[12px] text-[#9591A8]">{courseMods.length} modules · {p.total} lessons</div>
                    </div>
                    <div className="text-[20px] font-semibold text-[#7C5CDB] flex-shrink-0">{p.pct}%</div>
                  </div>
                  <div className="h-2.5 bg-[#E9E7EF] rounded-full overflow-hidden mb-3">
                  ...
                  {courseMods.length > 0 && (
                    <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${Math.min(courseMods.length, 4)}, 1fr)` }}>
                      {courseMods.map(m => (
                        <div key={m.id} className="bg-[#F7F7F9] rounded-md p-2 border border-[#E9E7EF]">
                          <div className="text-[11px] font-bold text-[#1A1523] mb-1 truncate">{m.title}</div>
                          <div className="h-1 bg-[#E9E7EF] rounded-full overflow-hidden">
                            <div className="h-full rounded-full" style={{ width: `${m.pct}%`, background: thumb.bar }} />
                          </div>
                          <div className="text-[10px] text-[#9591A8] mt-0.5 text-right">{m.pct}%</div>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          {quizResults.length > 0 && (
            <div className="bg-white border border-[#E9E7EF] rounded-2xl p-5 shadow-[0_2px_8px_rgba(15,13,26,0.06)] mb-6">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[15px] font-bold text-[#1A1523]">Quiz Score History</span>
                <div className="flex gap-1.5">
                  {(['all', ...chartCourses] as const).map(f => (
                    <button key={f} onClick={() => setChartFilter(f)}
                      className={`px-3 py-1 rounded-full text-[12px] font-bold border-none transition-all duration-[150ms] ease-[cubic-bezier(0.25,1,0.5,1)] ${chartFilter === f ? 'bg-[#7C5CDB] text-white' : 'bg-[#F7F7F9] text-[#9591A8] hover:bg-[#EDE8FF] hover:text-[#7C5CDB]'}`}>
                      {f === 'all' ? 'All' : f.split(' ')[0]}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex items-end gap-2.5 h-[140px] pb-6 relative">
                <div className="absolute bottom-6 left-0 right-0 h-px bg-[#E9E7EF]" />
                {filteredQuiz.map((q, i) => {
                  ...
                  const barBg   = isLow ? 'linear-gradient(to top,#DC2626,#F87171)' : isJs ? 'linear-gradient(to top,#6146C4,#7C5CDB)' : 'linear-gradient(to top,#E8580A,#FF6B35)'
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      ...
                      <div className="text-[10px] text-[#9591A8] font-bold text-center">Q{i + 1}</div>
                      <div className="text-[10px] text-[#9591A8] text-center">{q.score}%</div>
                    </div>
                  )
                })}
              </div>
              <div className="flex gap-4 mt-1">
                <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-sm inline-block" style={{ background: '#7C5CDB' }} /><span className="text-[11px] text-[#9591A8]">JavaScript</span></div>
                <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-sm inline-block" style={{ background: '#FF6B35' }} /><span className="text-[11px] text-[#9591A8]">HTML/CSS</span></div>
              </div>
            </div>
          )}

          {/* AI Recommendations */}
          <div className="rounded-2xl p-5 relative overflow-hidden mb-2" style={{ background: 'linear-gradient(135deg,#7C5CDB 0%,#6146C4 100%)' }}>
        ...
        <div className="w-[280px] flex-shrink-0 flex flex-col gap-4">
          ...
          <div className="bg-white border border-[#E9E7EF] rounded-2xl overflow-hidden shadow-[0_2px_8px_rgba(15,13,26,0.06)]">
            <div className="px-4 py-3.5 border-b border-[#E9E7EF]"><span className="text-[13px] font-bold text-[#1A1523]">Activity — Last 4 Weeks</span></div>
            <div className="px-4 py-3">
              <div className="grid grid-cols-7 gap-0.5 mb-0.5">
                {['M','T','W','T','F','S','S'].map((d, i) => <div key={i} className="text-center text-[9px] font-bold text-[#C4C0D4]">{d}</div>)}
              </div>
              ...
              <div className="flex items-center gap-1 mt-2 justify-end">
                <span className="text-[10px] text-[#9591A8]">Less</span>
                {[0,1,2,3,4].map(v => <div key={v} className="w-3 h-3 rounded-[2px]" style={{ background: heatmapColor(v) }} />)}
                <span className="text-[10px] text-[#9591A8]">More</span>
              </div>
            </div>
          </div>
          ...
          {/* Achievements */}
          <div className="bg-white border border-[#E9E7EF] rounded-2xl overflow-hidden shadow-[0_2px_8px_rgba(15,13,26,0.06)]">
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-[#E9E7EF]">
              <span className="text-[13px] font-bold text-[#1A1523]">Achievements</span>
              <span className="text-[11px] text-[#9591A8]">{Math.min(4, stats.lessonsCompleted > 9 ? 4 : stats.quizzesPassed > 0 ? 2 : 1)} / 6 earned</span>
            </div>
            <div className="p-4 grid grid-cols-2 sm:grid-cols-3 gap-2">
              {[
                ...
                <div key={i} className={`rounded-xl border px-2 py-2.5 text-center transition-all duration-[150ms] ease-[cubic-bezier(0.25,1,0.5,1)] hover:-translate-y-0.5 ${a.earned ? 'bg-[#EDE8FF] border-[#D4CAF7]' : 'bg-[#F7F7F9] border-[#E9E7EF] opacity-55'}`}>
                  <div className="text-[22px] mb-1">{a.icon}</div>
                  <div className="text-[10px] font-bold text-[#1A1523] leading-tight">{a.label}</div>
                  {!a.earned && <div className="text-[10px] text-[#9591A8] mt-0.5">🔒</div>}
                </div>
              ))}
            </div>
          </div>
          ...
          {stats.quizzesTotal > 0 && (
            <div className="bg-white border border-[#E9E7EF] rounded-2xl overflow-hidden shadow-[0_2px_8px_rgba(15,13,26,0.06)]">
              <div className="px-4 py-3.5 border-b border-[#E9E7EF]"><span className="text-[13px] font-bold text-[#1A1523]">Quiz Performance</span></div>
              <div className="px-4 py-4 flex items-center gap-4">
                <svg viewBox="0 0 36 36" className="w-20 h-20 flex-shrink-0" style={{ transform: 'rotate(-90deg)' }}>
                  <circle cx="18" cy="18" r="16" fill="none" stroke="#E9E7EF" strokeWidth="3.5" />
                  <circle cx="18" cy="18" r="16" fill="none" stroke="#7C5CDB" strokeWidth="3.5"
                    strokeDasharray={`${(stats.quizzesPassed / stats.quizzesTotal) * 100.53} 100.53`}
                    strokeLinecap="round" />
                  <text x="18" y="22" textAnchor="middle" fontFamily="Lato" fontWeight="900" fontSize="8" fill="#1A1523" style={{ transform: 'rotate(90deg)', transformOrigin: '18px 18px' }}>
                    {Math.round((stats.quizzesPassed / stats.quizzesTotal) * 100)}%
                  </text>
                </svg>
                <div className="flex flex-col gap-2">
                  <div><div className="text-[20px] font-semibold text-[#1A1523] tracking-tight">{stats.quizzesTotal}</div><div className="text-[11px] text-[#9591A8]">Quizzes taken</div></div>
                  <div><div className="text-[16px] font-bold text-[#16A34A]">{stats.quizzesPassed}</div><div className="text-[11px] text-[#9591A8]">Passed (≥60%)</div></div>
                </div>
              </div>
            </div>
          )}
          ...
          {/* Recent Activity */}
          <div className="bg-white border border-[#E9E7EF] rounded-2xl overflow-hidden shadow-[0_2px_8px_rgba(15,13,26,0.06)]">
            <div className="px-4 py-3.5 border-b border-[#E9E7EF]"><span className="text-[13px] font-bold text-[#1A1523]">Recent Activity</span></div>
            <div className="px-4 py-2">
              {recentActivity.length === 0 ? (
                <p className="text-[13px] text-[#9591A8] py-3">No activity yet.</p>
              ) : recentActivity.slice(0, 6).map((r, i, arr) => (
                <div key={r.id} className={`flex gap-3 py-2 ${i < arr.length - 1 ? 'border-b border-[#E9E7EF]' : ''}`}>
                  <span className="w-5 h-5 rounded-full bg-[#DCFCE7] flex items-center justify-center flex-shrink-0 mt-0.5 text-[9px]">✓</span>
                  <div className="min-w-0">
                    <div className="text-[12px] font-bold text-[#1A1523] truncate">{r.lessonTitle}</div>
                    <div className="text-[10px] text-[#9591A8]">
          ...
          {stats.streak > 0 && (
            <div className="bg-gradient-to-br from-orange-50 to-red-50 border border-orange-200 rounded-2xl p-4">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div><div className="text-[20px] font-semibold text-[#1A1523] tracking-tight">{stats.streak}</div><div className="text-[11px] text-[#9591A8]">Day streak</div></div>
              </div>
